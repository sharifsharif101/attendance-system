                        ...
                        <div class="mb-6 p-4 bg-yellow-50 dark:bg-yellow-900/20 rounded-lg">
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <input type="date" name="passport_expiry" value="{{ old('passport_expiry', $employee->passport_expiry?->format('Y-m-d')) }}" 
                                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 shadow-sm">
                                    @if($employee->passport_expiry)
                                        @php $passportDays = now()->diffInDays($employee->passport_expiry, false); @endphp
                                        @if($passportDays < 0)
                                            <p class="text-xs text-red-500 mt-1">🔴 منتهي منذ {{ abs($passportDays) }} يوم</p>
                                        @elseif($passportDays <= 30)
                                            <p class="text-xs text-red-500 mt-1">🔴 ينتهي خلال {{ $passportDays }} يوم</p>
                                        @elseif($passportDays <= 90)
                                            <p class="text-xs text-yellow-500 mt-1">⚠️ ينتهي خلال {{ $passportDays }} يوم</p>
                                        @endif
                                    @endif
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">رقم الإقامة</label>
                                    <input type="text" name="residency_number" value="{{ old('residency_number', $employee->residency_number) }}" 
                                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 shadow-sm">
                                </div>
                                <div>
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">تاريخ انتهاء الإقامة</label>
                                    <input type="date" name="residency_expiry" value="{{ old('residency_expiry', $employee->residency_expiry?->format('Y-m-d')) }}" 
                                        class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 shadow-sm">
                                    @if($employee->residency_expiry)
                                        @php $residencyDays = now()->diffInDays($employee->residency_expiry, false); @endphp
                                        @if($residencyDays < 0)
                                            <p class="text-xs text-red-500 mt-1">🔴 منتهية منذ {{ abs($residencyDays) }} يوم</p>
                                        @elseif($residencyDays <= 30)
                                            <p class="text-xs text-red-500 mt-1">🔴 تنتهي خلال {{ $residencyDays }} يوم</p>
                                        @elseif($residencyDays <= 90)
                                            <p class="text-xs text-yellow-500 mt-1">⚠️ تنتهي خلال {{ $residencyDays }} يوم</p>
                                        @endif
                                    @endif
                                </div>
                            </div>
                            ...
                            <div class="grid grid-cols-1 md:grid-cols-2 gap-4">
                                ...
                                <div>
                                    ...
                                    <select name="contract_type" class="w-full rounded-md border-gray-300 dark:border-gray-600 dark:bg-gray-800 dark:text-gray-200 shadow-sm">
                                        ...
                                        <option value="probation" {{ old('contract_type', $employee->contract_type) == 'probation' ? 'selected' : '' }}>تحت التجربة</option>
                                    </select>
                                </div>
                                <div>
                                    ...
                                    @if($employee->contract_expiry)
                                        @php $contractDays = now()->diffInDays($employee->contract_expiry, false); @endphp
                                        @if($contractDays < 0)
                                            <p class="text-xs text-red-500 mt-1">🔴 منتهي منذ {{ abs($contractDays) }} يوم</p>
                                        @elseif($contractDays <= 30)
                                            <p class="text-xs text-red-500 mt-1">🔴 ينتهي خلال {{ $contractDays }} يوم</p>
                                        @elseif($contractDays <= 90)
                                            <p class="text-xs text-yellow-500 mt-1">⚠️ ينتهي خلال {{ $contractDays }} يوم</p>
                                        @endif
                                    @endif
                                </div>
                            </div>
                                ...
                                @if($employee->photo)
                                    <div class="flex-shrink-0">
                                        <img src="{{ asset('storage/' . $employee->photo) }}" 
                                            alt="{{ $employee->name }}" 
                                            class="w-24 h-24 object-cover rounded-lg border-2 border-gray-300 dark:border-gray-600">
                                        <p class="text-xs text-gray-500 dark:text-gray-400 mt-1">الصورة الحالية</p>
                                    </div>
                                @endif
                                <div class="flex-1">
                                    <label class="block text-sm font-medium text-gray-700 dark:text-gray-300 mb-1">تغيير الصورة</label>
